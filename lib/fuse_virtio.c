...
#include "libvhost-user/libvhost-user.h"
...
static void *fv_queue_thread(void *opaque)
{
        ...
        while (1) {
               ...
               while (1) {
                       ...
                       vu_queue_push(dev, q, elem, qi->write_count);
...
int virtio_session_mount(struct fuse_session *se)
{
        ...
        if (strlen(se->virtio_socket_path) >= sizeof(un.sun_path)) {
                fprintf(stderr, "Socket path too long\n");
                return -1;
        }

        /* Poison the fuse FD so we spot if we accidentally use it;
         * DO NOT check for this value, check for se->virtio_socket_path
         */
        se->fd = 0xdaff0d11;
         ...
         */
        unlink(se->virtio_socket_path);
        strcpy(un.sun_path, se->virtio_socket_path);
        size_t addr_len = sizeof(un.sun_family) + strlen(se->virtio_socket_path);
